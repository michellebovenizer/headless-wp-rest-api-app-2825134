...
const sourceURL = 'http://wordpress-training.local';
...
function buildPost( postObject ) {
    ...
    // Add the post ID to the .post element so we know what post is being displayed.
    document.querySelector( '.post' ).setAttribute( 'data-id', postObject.id );
...
function getPost(navItem) {
    ...
    let queryURL = `${sourceURL}/wp-json/wp/v2/posts/${postID}?_embed`;
    fetch( queryURL )
...
function getLatestPosts() {
    ...
    // Build the query URL and try to fetch some JSON.
    let queryURL = `${sourceURL}/wp-json/wp/v2/posts/`;
    fetch( queryURL )
        .then( response => response.json() )
